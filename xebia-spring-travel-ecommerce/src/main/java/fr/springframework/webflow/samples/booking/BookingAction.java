...
import fr.springframework.webflow.samples.util.BookingActionController;
...
public class BookingAction extends MultiAction implements SelfNaming,
    ...
    private final Logger auditLogger = LoggerFactory
            .getLogger("fr.xebia.audit.Booking");
    ...
    public BookingAction(
            @Qualifier("CreditCardService") CreditCardService creditCardService,
        ...
        creditCardTypeByName.put("American Express",
    ...
    public ObjectName getObjectName() throws MalformedObjectNameException {
        return new ObjectName(Monitoring.JMX_DOMAIN
                + ":type=BookingAction,name=" + beanName);
    }

    @ManagedAttribute
    public long getTotalDurationInMillis() {
        return TimeUnit.MILLISECONDS.convert(getTotalDurationInNanos(),
    ...
    private Event processException(RequestContext context, Throwable t,
                                   String message) {
        HttpServletResponse response = (HttpServletResponse) ((ServletExternalContext) ExternalContextHolder
        ...
        context.getMessageContext().addMessage(
                new MessageBuilder().error().defaultText(message).build());
        return error();
    }
    ...
    public Event submitPayment(RequestContext context, Booking booking)
        ...
        try {
            ...
            creditCardService.purchase(
                    new MonetaryAmount(booking.getTotal(), Currency.getInstance("USD")),
                    booking.createOrder(),
    ...
    fr.xebia.ws.travel.antifraud.v1_0.Booking toAntiFraudBooking(Booking booking) {
        ...
        creditCard.setType(creditCardTypeByName.get(paymentCard.getCardType()
                .getType()));
